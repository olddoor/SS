        ...
        extends HibernateDaoSupport implements GenericDao<T, PK>
{
    ...
    public GenericHibernateDao()
    {
        ...
        Class c = getClass();
        Type t = c.getGenericSuperclass();//返回表示此 Class 所表示的实体（类、接口、基本类型或 void）的直接超类的 Type
        if (t instanceof ParameterizedType)  //参数化类型
        {
            Type[] p = ((ParameterizedType) t).getActualTypeArguments();//返回表示此类型实际类型参数的 Type 对象的数组。
	...
	public  int SaveAllTheObjects(List<T> objectsList)
	{
		Transaction tx=null; 
		try 
		{
     ...
     * @return List
     */
    protected List<T> getListForPage(final Class arg, final Criterion[] criterions,final int start, final int length) {
     List list = getHibernateTemplate().executeFind(new HibernateCallback() {
      public Object doInHibernate(org.hibernate.Session session)
        throws HibernateException, SQLException {
    ...
    public void setEntity(Object entity){
    	this.entityClass=(Class<T>) entity;
    }
